...
var injectFlight = function (document, plane) {
    ...
    for (var i = 0; i < stations.length; i++) {
        ...
        if (station.count == 1) {
            ...
            for (var j = 1; j <= station.count; j++) {
                station.weight += parseFloat(
                    document.getElementById(station.id + '-' + j).value,
                );
            }
        }
    }
...
var onUpdate = function (field, document) {
    if (field.id.indexOf('gallons') != -1) {
        ...
        document.getElementById(field.id.replace('gallons', 'tank')).value =
            gallons * 6;
    }
...
var updateFlight = function (document) {
    ...
    document.getElementById('total-weight').innerHTML =
        'Ramp weight: ' + plane.totals.weight.toFixed(2) + ' lbs';
    drawGraphs(document);
};
