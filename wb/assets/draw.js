...
var convertPointForGraph = function (point, ranges) {
    return {
        x: Math.round(
            ((point.x - ranges.min.x) * 100) / (ranges.max.x - ranges.min.x),
        ),
        y: Math.round(
            80 -
                ((point.y - ranges.min.y) * 80) / (ranges.max.y - ranges.min.y),
        ),
    };
};
...
var drawWeightAndBalance = function (document, plane) {
    ...
    var ramp = convertPointForGraph(
        { x: plane.totals.moment / 1000, y: plane.totals.weight },
        diagram.ranges,
    );
    var zerofuel = convertPointForGraph(
        { x: plane.zerofuel.moment / 1000, y: plane.zerofuel.weight },
        diagram.ranges,
    );
...
var drawCG = function (document, plane) {
    ...
    var ramp = convertPointForGraph(
        {
            x: plane.totals.moment / plane.totals.weight,
            y: plane.totals.weight,
        },
        diagram.ranges,
    );
    var zerofuel = convertPointForGraph(
        {
            x: plane.zerofuel.moment / plane.zerofuel.weight,
            y: plane.zerofuel.weight,
        },
        diagram.ranges,
    );
    drawLine(document, element, ramp, zerofuel, 'actual');
...
var drawPointOnEnvelope = function (
    document,
    element,
    point,
    className,
    ...
    drawCircle(document, element, point, {
        className: className,
        radius: '2',
        id: id + '-circle',
    });
...
var drawStation = function (document, element, ranges, station) {
    var lineEnd = convertPointForGraph(
        { x: (station.maxWeight * station.arm) / 1000, y: station.maxWeight },
        ranges,
    );
    drawLine(
        document,
    ...
    drawCircle(document, element, center, {
        id: 'loading-' + station.id,
    ...
    drawLabel(
        ...
        lineEnd,
        station.id,
        'loading-station-' + station.id + '-text',
    );
};
