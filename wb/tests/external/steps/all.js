...
When(
    'I access the home page with flight info {string}',
    function (encoded, done) {
        browser.visit(
            'http://localhost:' + server.port + '/?flight=' + encoded,
            () => {
                setTimeout(() => {
                    done();
                }, 300);
...
Then("I see the normal envelope's points: {string}", function (value) {
    browser.assert.attribute(
        '#envelope polyline.normal-category',
        'points',
        value,
    );
});
Then("I see the utility envelope's points: {string}", function (value) {
    browser.assert.attribute(
        '#envelope polyline.utility-category',
        'points',
        value,
    );
});
...
Then(
    'I see the {string} point at {string} with label {string}',
...
Then(
    'I see the {string} loading line from origin to {string} with mark at {string}',
    function (station, max, mark) {
        let parts = max.split(',');
        let x = parts[0];
        let y = parts[1];
        ...
        browser.assert.attribute(
            '#load text#loading-station-' + station + '-text',
            'x',
            x,
        );
        browser.assert.attribute(
        ...
        y = parts[1];
        browser.assert.attribute(
            '#load circle#loading-' + station + '.station-' + station,
            'cx',
            x,
        );
        browser.assert.attribute(
        ...
        let element = browser.document.querySelector(
            '#load circle#loading-' + station,
        );
...
Then(
    "I see the CG graph displays the normal envelope's points: {string}",
    function (value) {
        browser.assert.attribute(
            '#cg polyline.normal-category',
            'points',
            value,
        );
    },
...
Then(
    "I see the CG graph displays the utility envelope's points: {string}",
    function (value) {
        browser.assert.attribute(
            '#cg polyline.utility-category',
            'points',
            value,
        );
    },
...
Then(
    'I see the CG graph displays the {string} point at {string} with label {string}',
    function (point, value, label) {
        let x = value.split(',')[0].trim();
        let y = value.split(',')[1].trim();
...
Then(
    'I see that the weight of station {string} is {string}',
